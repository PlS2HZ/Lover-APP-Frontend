...
const MindGame = () => {
  ...
  const fetchLevels = async () => {
    setLoading(true);
    try {
      // ✅ ดึงข้อมูลด่านพร้อม sessions เพื่อหา Best Time (time_spent)
      ...
      <div className="max-w-2xl mx-auto p-4 space-y-4 mt-4">
        {loading ? (
          ...
          <div className="bg-white border-2 border-dashed border-slate-200 rounded-[2.5rem] p-12 text-center">
            ...
            // ✅ หา Best Time จาก sessions (time_spent)
            const bestTime = level.sessions?.reduce((min, s) => 
              (s.status === 'finished' && (s.time_spent < min || min === 0) ? s.time_spent : min), 0);

            return (
              <div key={level.id} className={`relative bg-white rounded-[2rem] p-5 shadow-xl shadow-slate-100 border-2 transition-all ${isOwner ? 'border-slate-100 opacity-90' : 'border-rose-50 hover:border-rose-200'}`}>
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex gap-2">
                    {/* ✅ ปุ่มลบด่านสำหรับเจ้าของ */}
                    {isOwner && (
                      <button onClick={() => handleDeleteLevel(level.id, level.secret_word)} className="bg-slate-100 text-slate-400 w-12 h-12 rounded-2xl flex items-center justify-center hover:bg-rose-50 hover:text-rose-500 transition-all active:scale-90">
                        <Trash2 size={20} />
                      </button>
                    )}
                    {!isOwner && (
                      <button onClick={() => handlePlayNow(level)} className="bg-gradient-to-br from-rose-400 to-pink-600 text-white w-12 h-12 rounded-2xl flex items-center justify-center shadow-lg active:scale-90 transition-all">
